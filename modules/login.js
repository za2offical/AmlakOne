...
router.post('/verify', async (req, res) => {
    try {
        ...
        // بررسی اعتبار کاربر عادی از دیتابیس
        ...
        if (user.lockoutUntil && Date.now() > user.lockoutUntil) {
            ...
            // مقدار جدید را از دیتابیس بگیر
            const freshUser = await getUserByUsername(username);
            user.failedLoginAttempts = freshUser.failedLoginAttempts;
            user.lockoutUntil = freshUser.lockoutUntil;
        }
