...
router.get('/', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'خطا در دریافت درخواست‌های 3D' });
    }
});
...
router.get('/:id', async (req, res) => {
    try {
        ...
        console.error('Error fetching 3D request:', error);
        res.status(500).json({ error: 'خطا در دریافت درخواست 3D' });
    }
});
...
router.post('/', authenticateToken, async (req, res) => {
    try {
        ...
        await create3DRequest(requestData);
        res.status(201).json({ message: 'درخواست 3D با موفقیت ایجاد شد', id: requestData.id });
    } catch (error) {
        console.error('Error creating 3D request:', error);
        res.status(500).json({ error: 'خطا در ایجاد درخواست 3D' });
    }
});
...
router.put('/:id', authenticateToken, async (req, res) => {
    try {
        const result = await update3DRequest(req.params.id, req.body);
        if (result.changes === 0) {
            return res.status(404).json({ error: 'درخواست یافت نشد' });
        }
        res.json({ message: 'درخواست 3D با موفقیت به‌روزرسانی شد' });
    } catch (error) {
        console.error('Error updating 3D request:', error);
        res.status(500).json({ error: 'خطا در به‌روزرسانی درخواست 3D' });
    }
});
...
router.delete('/:id', authenticateToken, async (req, res) => {
    try {
        const result = await delete3DRequest(req.params.id);
        if (result.changes === 0) {
            return res.status(404).json({ error: 'درخواست یافت نشد' });
        }
        res.json({ message: 'درخواست 3D با موفقیت حذف شد' });
    } catch (error) {
        console.error('Error deleting 3D request:', error);
        res.status(500).json({ error: 'خطا در حذف درخواست 3D' });
    }
});
